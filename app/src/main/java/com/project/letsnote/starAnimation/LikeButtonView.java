package com.project.letsnote.starAnimation;
...
public class LikeButtonView extends FrameLayout implements View.OnClickListener {
    ...
    public void onClick(View v) {
        ...
        if(PerfilAjeno.esAmigo) {
            ...
            final Firebase refAmigos = new Firebase("https://letsnote.firebaseio.com/users").child(PerfilAjeno.keyUser).child("seguidores").child(Login.FIREBASEKEY);
            refAmigos.setValue(false);
            final Firebase refAmigos2 = new Firebase("https://letsnote.firebaseio.com/users").child(Login.FIREBASEKEY).child("seguidos").child(PerfilAjeno.keyUser);
            refAmigos2.setValue(false);
            int amigosAnterior = Integer.parseInt(PerfilAjeno.seguidores.getText().toString());
            amigosAnterior--;
            PerfilAjeno.seguidores.setText(String.valueOf(amigosAnterior));
            PerfilAjeno.esAmigo = false;
        }else if(!PerfilAjeno.esAmigo){
            ivStar.setImageResource(R.drawable.ic_star_rate_on);
            PerfilAjeno.tvSeguir.setText("Siguiendo");
            final Firebase refAmigos = new Firebase("https://letsnote.firebaseio.com/users").child(PerfilAjeno.keyUser).child("seguidores").child(Login.FIREBASEKEY);
            refAmigos.setValue(true);
            final Firebase refAmigos2 = new Firebase("https://letsnote.firebaseio.com/users").child(Login.FIREBASEKEY).child("seguidos").child(PerfilAjeno.keyUser);
            refAmigos2.setValue(true);
            int amigosAnterior = Integer.parseInt(PerfilAjeno.seguidores.getText().toString());
            amigosAnterior++;
            PerfilAjeno.seguidores.setText(String.valueOf(amigosAnterior));
            PerfilAjeno.esAmigo = true;
        }

        //ivStar.setImageResource(PerfilAjeno.esAmigo ? R.drawable.ic_star_rate_on : R.drawable.ic_star_rate_off);
